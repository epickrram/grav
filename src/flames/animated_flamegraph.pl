...
SVG
	...
	sub background {
		...
		$w = sprintf "%0.1f", $w;
		$h = sprintf "%0.1f", $h;
		$self->{svg} .= qq/<rect x="0" y="0" width="$w" height="$h" fill="url(#background)"\/>\n/;
	}

	sub filledRectangleStart {
		my ($self, $fill, $x1, $x2, $y1, $y2) = @_;
		$x1 = sprintf "%0.1f", $x1;
		$x2 = sprintf "%0.1f", $x2;
...
#print "x1: $x1 w : $w\n";		
	...
	sub animateWidth {
		...
		my @d = @{ $dRef };
		my @stime = @{ $stimeRef };
		my $oldw = sprintf "%0.1f", $x2 - $x1;
		my $j;
		for ($j = 0; $j < scalar @d; $j++) {
			$x1 = $xpad + $stime[$j] * $widthpertime;
...
sub calculate_stime {
	...
	for (; $i < scalar @d; $i++) {
#print "$i - \ndepth $depth\nprev $prev\n";		
		my $prevStack = (defined $Tmpetime[$i][$depth]) ? $Tmpetime[$i][$depth] : 0;
		my $value = (defined $Tmpstime[$i][$prev]) ? $Tmpstime[$i][$prev] : 0;
#print "prevInStackS $prevStack \nprevInDepthE  $value \n";
		$value = max($value, $prevStack);
		$Tmpstime[$i][$depth] = $value;
	    $Tmpetime[$i][$depth] = $value + $d[$i];
#print "$i d $depth TEMP  E $Tmpetime[$i][$depth]  S $Tmpstime[$i][$depth] \n";	    
		push(@stime, $value);
	}
#print "STIME $depth - @stime\n";
	return @stime;
}
...
#print "last @$last this @$this v=$v d=@d\n";
...
#print "range: same $len_same a $len_a b $len_b\n";
	for ($i = $len_a; $i >= $len_same; $i--) {
		my $k = "$last->[$i];$i";
		# a unique ID is constructed from "func;depth;etime";
		# func-depth isn't unique, it may be repeated later.
		my @totalDelta = @{$Tmp{$k}->{delta}};
		my @totalStime = @{$Tmp{$k}->{stime}};
#print "final STIME for $k;$v = @totalStime \n";	
...
#print "set to Node $k;$v = @{$Node{\"$k;$v\"}->{delta}}\n";
	...
	for ($i = 0; $i <= $len_b; $i++) {
		...
		if (defined $Tmp{$k}->{delta}) {
			@sum = array_sum(\@{$Tmp{$k}->{delta}}, \@d);
#print "sum of   @{$Tmp{$k}->{delta}} and @d is @sum\n";	
			for ($j = 0; $j < scalar @d; $j++) {
#print "TEMP $j $i ETIME was $Tmpetime[$j][$i]\n";
				$Tmpetime[$j][$i] += $d[$j];
#print "TEMP $j $i ETIME now $Tmpetime[$j][$i]\n";
			}
		}
		else {
			@sum = @d;
		...
		$Tmp{$k}->{delta} = [@sum];
#print "Tmp DELTA for $k = @sum \n";
		if ($i >= $len_same) {
			$Tmp{$k}->{stime} = [calculate_stime($i, @sum)];		
#my @xxx = @{$Tmp{$k}->{stime}};
#print "Tmp STIME for $k = @xxx\n";
		}
	}
    return $this;
}
...
while (my ($id, $node) = each %Node) {
	...
	die "missing start for $id" if not @stime;

	my @sorted = sort { $a <=> $b } @stime;
	if (($etime-$sorted[0]) < $minwidth_time) {
		delete $Node{$id};
		next;
	}
...
INC
$im->include($inc);
$im->background($imagewidth, $imageheight);
...
#print "pad $xpad  widthpertime $widthpertime \n";
...
#print "func $func  depth $depth -  stime  @stime  delta @d \n";
	...
	my $x1 = $xpad + $stime[0] * $widthpertime;
	my $x2 = $xpad + ($stime[0] + $d[0]) * $widthpertime;
	my $y1 = $imageheight - $ypad2 - ($depth + 1) * $frameheight + $framepad;
...
#print "$func depth $depth s @stime d @d\n";

	$im->filledRectangleStart($color, $x1, $x2, $y1, $y2);

	$im->animateWidth($x1, $x2, \@d, \@stime);

	$im->filledRectangleEnd();
